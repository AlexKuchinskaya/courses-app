...
export const useAuthor = () => {
  ...
  const getCurrentAuthor = (list: AuthorList, id: string) => list.find((author) => author.idAuthor === id);

  const onCreateAuthor = () => {
    console.log(authorValue)
    if(!authorValue.trim().length) return;

    const isAuthorExist = Boolean(authorsList.filter((author)=> author.authorName === authorValue).length) ||
    Boolean(courseAuthors.filter((author)=> author.authorName === authorValue).length);
  ...
  const onAddAuthorToCourseList = (id: string) => {
    ...
    if (currentAuthor) {
      const filteredAuthorsList = authorsList.filter((author) => author !== currentAuthor);
  ...
  const onDeleteFromCourseAuthors = (id: string) => {
    ...
    if (currentAuthorCourse) {
      const filteredCourseAuthorsList = courseAuthors.filter((author) => author !== currentAuthorCourse);
